...
void bfs(uint64_t *level, uint64_t *buffer, uint64_t buffer_size, uint64_t *index_of_node, int my_rank, int procs, int scale, uint64_t *parent_array, double *time_allwork, double *time_allreduce, double *time_parentreduce){
    uint64_t nodes = pow(2,scale);
    uint64_t nodes_owned = nodes / procs;
    ...
    while (oneChildisVisited){
        oneChildisVisited = 0;
        #pragma omp parallel default(shared) private(position_actual, position_neigh, level_local)
        {
            level_local = (uint64_t *) calloc(nodes / BITS, sizeof(uint64_t));
            memcpy((void*)level_local,(void*)level, (nodes / BITS)*sizeof(uint64_t));
                ...
                while (not_visited[i]){
                    ...
                    if ((i*BITS+position_actual) == nodes_owned -1){
                        for (; j < buffer_size; j++){
                            ...
                            if (position_neigh & ~level_local[(buffer[j]/BITS)]){
                                parent_array[buffer[j]] = (uint64_t) nodes_owned*my_rank+(i*BITS+position_actual)+1;
                                level_local[(buffer[j]/BITS)] = level_local[(buffer[j]/BITS)] | position_neigh;
                                oneChildisVisited = 1;
                            }
                        }
                    }else{
            ...
            #pragma omp critical
            {
                for (i = 0; i < (nodes / BITS); i++){
                    level[i] = level[i] | level_local[i];
                }
            }
            free(level_local);
        }
