...
$\textbf{PARALLEL-BFS-ALLTOALL}$($G_{p}$,s)
	...
	$\textbf{while}$ (isNewLevel == 1)
		isNewLevel = 0
		$\textbf{for}$ each u $\in$ FS
			$\textbf{if}$ $\pi$[u $\textit{\%}$ n/p] == NIL
				$\pi$[u $\textit{\%}$ n/p] = u.$\pi$
				$\textbf{for}$ each w $\in$ $G_{p}$.Adj[u]
					$\textbf{if}$ v[w] == 0
						$p_{i}$ = $\textit{FIND\_OWNER}$(w)
						$NS_{p_{i}}$ = $NS_{p_{i}}$ $\cup$ {w}
						isNewLevel = 1
		NS = $\emptyset$
		FS = $\emptyset$
		$\textbf{for}$ 0 $\leq$ $p_{i}$ $<$ p $\textbf{do}$
	...
	GATHER($\pi$, $p_{0}$)
\end{lstlisting}
Algorithmus~\ref{par_alltoall} wird wie Algorithmus~\ref{par_allvisited} mit dem eindimensional zerlegtem Teilgraphen \lstinline{$G_{p}$} und dem Startknoten \lstinline{s} von jedem verfügbaren Prozessor aufgerufen.
