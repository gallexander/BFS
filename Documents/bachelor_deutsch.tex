...
Die Idee hinter diesem Algorithmus war neben der eindimensionalen Zerlegung des Graphen und damit parallelen Abarbeitung der Levels, dass die Prozessoren mehr Informationen über die anderen Prozessoren haben und deren Knoten. Bei vielen parallelen Algorithmen der Breitensuche, wie auch beim Algorithmus in Kapitel~\ref{sec:bfs_alltoall} ist es so, dass die Prozessoren nur Informationen über die lokal gespeicherten Knoten besitzen und nur anhand dieser Informationen Entscheidungen treffen müssen, wodurch es oft zu Redundanzen in der Kommunikation kommt. Zum Beispiel ist es einem Prozessor bei diesen Algorithmen nicht bekannt, ob ein Knoten, der nicht zu den lokalen gehört, bereits besucht wurde, da dies nur lokal von den Prozessoren gespeichert wird. Deshalb muss jedes Mal beim einem fremden Knoten zwischen den Prozessoren kommuniziert werden, ob dieser Knoten bereits in einem vorherigen Level besucht wurde.
