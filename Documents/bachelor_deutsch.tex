...
Außerdem wichtig für die Auswertung der Ergebnisse ist der verwendete Graph und wie dieser generiert wurde. Wir haben uns für Stochastische Kronecker Graphen entschieden, da eine rasche Generierung dieser Graphen möglich ist und man das Aussehen beziehungsweise die Verteilung des Graphen mittels einer Initiator Matrix stark beeinflussen kann. Diese Informationen beziehen wir auf die Arbeit von Leskovec, Chakrabarti, Kleinberg, Faloutsos und Ghahramani~\cite{kronecker} und verweisen auch auf diese Arbeit für ausführlichere Informationen, da dies sonst den Rahmen dieser Arbeit sprengen würde. Diese Generierung ist sehr ähnlich zu der des R-MAT Generator, und wir setzen die Werte in der Initiator Matrix genauso wie die Parameter \(a\), \(b\), \(c\) und \(d\) des R-MAT Generator auf \(0.57, 0.19, 0.19, 0.05\) um einen Graphen mit einem sehr kleinen Durchmesser(\(\leq 10)\) zu erhalten. Dabei bestimmen diese Parameter, wie die Verteilung der Kanten im Graphen ist. Weiters werden die gleichen Parameter auch für das später vorgestellte Projekt Graph500 für die Generierung der Graphen verwendet. Aufgrund der unterschiedlichen Verteilung der Kanten im Graphen, würden die einzelnen MPI Prozesse unterschiedlich viele Kanten zugewiesen bekommen, wodurch sich eine ungleichmäßige Lastverteilung ergeben würde. Um einen Lastausgleich zwischen den MPI Prozessen durchzuführen, haben wir uns deshalb entschieden die Identifikationsnummern der Knoten pseudozufällig zu mischen, damit alle MPI Prozesse ähnlich viele Kanten besitzen.\\
