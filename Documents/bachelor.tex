...
The first approach of splitting the BFS is to split the nodes of each level to the processors and every processor of the system just visit the neighbours of the owned nodes. The problem with this approach is that the communication costs are pretty high, because it's necessary to split the work every round (level) of the BFS algorithm.\\
So the second and also chosen approach is to divide all the existing nodes and corresponding edges of the graph to the processors. With this approach the highest amount of communication is just at the beginning of the algorithm. After that the cores just have to communicate, which node is in the current level. For instance if I have the root node at processor 0 the whole work of level 0 is just at the processor with the number 0. But now processor 0 has to tell the other processors which nodes he has visited. So these processors have to prove if they have one of these visited nodes stored in their node list. If they have, it's their turn for the next level. Figure is in progress....
