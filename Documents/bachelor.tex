...
In this section we will show some commands from the pseudo code in \ref{sec:core} and a possibility how to implement it in the programming language C with MPI. A good description of MPI is given by Rauber and R{\"u}nger \cite{rauber}. \\
...
\end{ccode}
\cinline/level/ is a bitmap of type \cinline/uint64_t/ and contains the information, which nodes of the entire graph are visited and which not. cinline/buffer/ is an array and saves the neighbours of the nodes, which are assigned to the processor calling the function.
The function 
\begin{ccode}
